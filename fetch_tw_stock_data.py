...
def fetch_price_data(stock_id: str, start_date: str, end_date: str = None,
                      token: str = "", adjusted: bool = False) -> pd.DataFrame:
    """
    抓取每日股價 OHLCV。

    【重要】TaiwanStockPriceAdj（還原股價）是 FinMind 的 backer/sponsor 付費會員
    限定資料集，一般註冊會員（不管有沒有帶token）呼叫都會得到 400 錯誤
    （已實測確認）。所以這裡預設 adjusted=False，用免費的 TaiwanStockPrice。

    股票的除權息還原，本專案改用 fetch_tw_price_native.py 裡
    fetch_stock_price_range_adjusted()（直接爬證交所+自己算還原股價，完全免費），
    不要在這裡把 adjusted 設成 True，除非你已經付費升級 FinMind 的 backer/sponsor 會員。
